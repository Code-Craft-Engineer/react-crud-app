...
const UpadateForm = ({user,setUser}) => {
  const { id } = useParams();
  const data = user.find((value) => value.id === Number(id));
  const [newObj,setnewObj]=  useState({id:99999,fname:"prakhar"})
  console.log(data)
  const handleSubmit = (e) =>{
    e.preventDefault();
    const newArray = user.map((value) => value.id === Number(id) ? {...value,...newObj} : value);
    setUser(newArray)
    console.log(user )
  }
  return (
